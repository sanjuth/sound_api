...
async def detect_return_json_result(file: bytes = File(...)):
    ...
    result= {"disease_lstm": {"prediction": results[0],"confidence": str(results[1])} ,
    "disease_gru": {"prediction": results[2],"confidence": str(results[3])},
    "rr_lstm": {"prediction": res[0],"confidence": str(res[1])},
    "rr_gru": {"prediction": res[2],"confidence": str(res[3])}}
